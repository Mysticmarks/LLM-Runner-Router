...
const router = new LLMRouter({
  ...
  openrouter: {
    ...
    // Cost Management
    costLimit: 10.00, // Monthly limit in USD
...
class OpenRouterErrorHandler {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (this.shouldRetry(error, attempt)) {
          ...
          // Try different model on certain errors
  ...
  adjustRequestForRetry(request, error) {
    // Try fallback models for unavailable services
    const fallbackMap = {
      'anthropic/claude-3-opus': 'anthropic/claude-3-sonnet',
      'openai/gpt-4': 'openai/gpt-3.5-turbo',
      'meta-llama/llama-3-70b': 'meta-llama/llama-3-8b'
    };
    
    if (fallbackMap[request.model]) {
      return { ...request, model: fallbackMap[request.model] };
    }
...
| 503 | Service Unavailable | Try fallback model |
